...
mlist = {'m1044', 'm1045', 'm1048', 'm1049', 'm1092', 'm1094', 'm1859', 'm1873', 'm1613'}; 
...
for f = 1:length(mlist)
    ...
    for ff = 1:length(sessions)
        ...
        searchStr = [header, '*red*chunk*.mat']; 
...
% The number of most common valid (non-NaN) pixels 
mostCommonValNumPix = sum(~ismember(1:gp.originaldimensions(1)*gp.originaldimensions(2), mostCommonNanpxs)); 
% (Option 1) Correct for any mismatch in the dimensions of the vectorized spatiotemporal motifs (W)
for ii = 1:numel(wC)
    if ~isequal(nanpxsC{ii}, mostCommonNanpxs) && ~isequal(wC{ii}, mostCommonValNumPix)
        wC{ii} = matchMotifDims(wC{ii}, nanpxsC{ii}, mostCommonNanpxs, gp); 
    end
end
% (Option 2) Use Ws with matching dimensions without correcting for mismatches
%is_equal_nanpxsC = cellfun(@(x) isequal(x, mostCommonNanpxs), nanpxsC);
%wCV = wC(is_equal_nanpxsC); 
%nanpxsCV = nanpxsC(is_equal_nanpxsC); 

% save
save(fullfile(fileSaveDir, 'clusterW_input_CAmotifs_072125'), 'wC', 'nanpxsC', 'idC', 'skippedFiles', '-v7.3'); % Also backup at '/Volumes/buschman/Rodent Data/dualImaging_parkj/motif_data_backup'
...
[W_basisC, W_aligned, kval, ovr_q, cluster_idxC, idx_knn, tcorr_mat, handles, lag_mat, lags, nanpxs, shiftC] = ClusterW_multiLevels(wC, gp, mostCommonNanpxs); 

% temporarily save ClusterW output
save(fullfile(fileSaveDir, 'clusterW_output_CAmotifs_072125.mat'), 'W_basisC', 'W_aligned', 'kval', 'ovr_q', 'cluster_idxC', 'idx_knn', 'tcorr_mat', 'lag_mat', 'lags', 'nanpxs', 'shiftC', '-v7.3');
