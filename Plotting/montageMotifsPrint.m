...
% montageMotifsPrint  Display and optionally print motif montages.
%
% Usage:
...
%   montageMotifsPrint(motif_w, nanpxs, 'smoothLogic', true, 'gaussianSigma', 1)
%
% Inputs:
...
%
% Notes:
%   - Gaussian smoothing is applied after motif frames are reconstructed into
%     image format, but before percentile/global scaling.
...
% New: Gaussian smoothing options
...
for figIdx = 1:nFigures
    ...
    for i = 1:motifsThisFig
        ...
        if P == 64 * 64
            ...
            motif = reshape(squeeze(motif_w_sel(:, motifIdxLocal, :)), ...
                64, 64, []);
            
        else
            
            motif = conditionDffMat( ...
                squeeze(motif_w_sel(:, motifIdxLocal, :))', ...
                nanpxs);
        end
        ...
        % -----------------------------------------------------------------
        %
        % Apply smoothing before scaling.
        % This preserves the raw motif structure first, then rescales display.
