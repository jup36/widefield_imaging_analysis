function figH = plot_LK_PEV_heatmap_perAnimal(pevSummaryT, varargin)
% plot_LK_PEV_heatmap_perAnimal
%
% Visualize mean PEV across L-K_input combinations per animal.
%
% Input:
%   pevSummaryT : table with variables:
%       mouseID, sessionID, L, K_input, meanPEV
%
...
%   'savePrefix'  : filename prefix. Default = 'PEV_LK_heatmap'
%   'valueFormat' : text format for tile values. Default = '%.3f'
...
addParameter(p, 'savePrefix', 'PEV_LK_heatmap', @(x) ischar(x) || isstring(x));
addParameter(p, 'valueFormat', '%.3f', @(x) ischar(x) || isstring(x));
...
%% Basic checks
requiredVars = {'mouseID', 'L', 'K_input', 'meanPEV'};
...
for m = 1:nMice
    ...
    pevMat = nan(numel(lags), numel(Ks));
    ...
    for iL = 1:numel(lags)
        for iK = 1:numel(Ks)
            ...
            if any(rowI)
                ...
                pevMat(iL, iK) = mean(thisT.meanPEV(rowI), 'omitnan');
            end
        end
    end
...
if showColorbar
    try
        ...
        if ~isempty(validAx)
            ...
            % Do not call cb.Label.String or ylabel(cb,...)
            % These trigger errors/warnings in older MATLAB setups.
        end

        sgtitle('Mean PEV across lag-K combinations per animal', ...
            'FontWeight', 'bold');

    catch ME
        warning('Colorbar failed: %s. Continuing without colorbar.', ME.message);

        sgtitle(sprintf('Mean PEV across lag-K combinations per animal; color scale %.3f to %.3f', ...
...
if figSaveLogic
    ...
    % exportgraphics is nice but can fail on older setups; fallback to print.
    try
        exportgraphics(figH, [saveNameBase '.png'], 'Resolution', 300);
    catch
