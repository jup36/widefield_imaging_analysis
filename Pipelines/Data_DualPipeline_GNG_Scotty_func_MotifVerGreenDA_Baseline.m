function Data_DualPipeline_GNG_Scotty_func_MotifVerGreenDA_Baseline(filePathImg, fileKeyword)
...
parameter_class = 'general_params_dual_L10';
gp = loadobj(feval(parameter_class)); 

% Get a cell array (fnC) that contains filePaths to 'dff_combined' matfiles
filePathImg_dff = cell2mat(GrabFiles_subfolders(fileKeyword, filePathImg)); % use GrabFiles_sort_trials to sort both files and folders
if isempty(filePathImg_dff)
   return 
end
...
folderName = sprintf("%s_motif", fileheader); 
...
fileName = sprintf("%s_processed%s", fileheader, fileKeyword);  
file_processed = fullfile(file_processed_dir, fileName); % file SAVE DIR

%deconvolve and split the data
script_name = WriteBashScriptWinScotty(sprintf('%d',1), ...
    'ProcessAndSplitDataGreenDA', ...
    {ConvertWinToBucketPath(filePathImg_dff), ...
    ConvertWinToBucketPath(file_processed), ...
    'general_params_dual_L10'}, ...
    {"'%s'","'%s'","'%s'"}, ...
...
[swarm_id, swarm_motifs] = FitMotifs_ScottySwarm_chunks(file_processed, temp_job_id, s_conn, 'general_params_dual_L10', 1); % 1 chunk (fixed; 1 train and 1 test set)
