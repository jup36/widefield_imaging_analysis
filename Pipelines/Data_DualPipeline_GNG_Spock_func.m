...
for cur_fold = 1:numel(folder_list_raw)
    ...
    for cur_file = 1:numel(file_list_raw)
        ...
        script_name = WriteBashScriptWin(sprintf('%d_%d', cur_fold, 1), ...
        ...
        if cur_file ~= numel(file_list_raw)s
    ...
    script_name = WriteBashScriptWin(sprintf('%d_%d_combine', cur_fold, cur_file), ...
