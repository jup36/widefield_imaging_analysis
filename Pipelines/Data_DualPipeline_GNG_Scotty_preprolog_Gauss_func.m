function Data_DualPipeline_GNG_Scotty_preprolog_Gauss_func(filePathImg, fileKeyword, varargin)
...
%% Manual Portion
[file_list_first_stack, folder_list_raw] = GrabFiles_subfolders(fileKeyword, filePathImg);

% Determine channel-specific folder keyword
if contains(fileKeyword, 'red', 'IgnoreCase', true)
    chFolderKeyword = '_img_1_red';
elseif contains(fileKeyword, 'green', 'IgnoreCase', true)
    chFolderKeyword = '_img_1_green';
else
    error('fileKeyword must contain either ''red'' or ''green''.');
end

% First try to find prepro_log from the corresponding task folder
filePathImgTask = regexprep(filePathImg, 'baseline', 'task');

preprologPath_task = GrabFiles_sort_trials(chFolderKeyword, 0, {filePathImgTask});

path_prepro_log = {};

if ~isempty(preprologPath_task)
    path_prepro_log = GrabFiles_sort_trials('prepro_log', 0, preprologPath_task(1));
end

% If task prepro_log is unavailable, fall back to baseline folder
if isempty(path_prepro_log)

    preprologPath_base = GrabFiles_sort_trials(chFolderKeyword, 0, {filePathImg});

    if ~isempty(preprologPath_base)
        path_prepro_log = GrabFiles_sort_trials('prepro_log', 0, preprologPath_base(1));
    end
end

% Optional: report what happened
if isempty(path_prepro_log)
    warning('No prepro_log found in task or baseline folders.');
    return
else
    fprintf('Using prepro_log:\n%s\n', path_prepro_log{1});
end
...
for cur_fold = 1:numel(folder_list_raw)
    ...
    for cur_file = 1:numel(file_list_raw)
        input_val = {ConvertWinToBucketPath(file_list_raw{cur_file}), ConvertWinToBucketPath(path_prepro_log{1})};
