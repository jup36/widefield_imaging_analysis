function Data_DualPipeline_passive_Spock_func(filePathImgColor, varargin)
%This script runs preprocessing of an ome.tiff image stack. 
% filePathImgColor must be a folder that contains ome.tiff stack(s). 

p = parse_input_ctxWideImageDualPreprocess(filePathImgColor, varargin);
%p = parse_input_ctxWideImageDualPreprocess(filePathImgColor, {'redoManual', false, 'dffMethod', 'movingavg', 'winF', 30, 'channelToProcess', 'green'}); 
...
opts = ConfigurePreProcessingDual('crop_w', 400, 'crop_h', 400, 'x_bregma_margin', 234, 'y_bregma_margin', 184, ...
    'fps', 10, 'vasc_std', 1.5, 'save_uncorrected', 0, 'method', p.Results.dffMethod, 'method_window', p.Results.winF);
opts.x_bregma_margin = 280-(540-opts.crop_w)/2; % Adjust to match the brain-outline mask to new image size (PNI284)
opts.y_bregma_margin = 280-(540-opts.crop_h)/2; % Adjust to match the brain-outline mask to new image size
...
%% Manual Portion
[file_list_first_stack, folder_list_raw] = GrabFiles_sort_trials('_ome', 0, {filePathImgColor}); % use GrabFiles_sort_trials to sort both files and folders 
