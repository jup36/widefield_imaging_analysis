...
stimOnFig_learn_RS = plotMeanSem(rezRS_learn.meanStimOnDffGo, rezRS_learn.semStimOnDffGo, stimOn_timepts1, {header1, header2});
...
print(fullfile(figSaveDir, strcat(mouseId1, '_', date1, '_', date2, '_dff_cueOn_hit_early_late_RS')), '-dpdf', '-vector', '-bestfit')
%close(stimOnFig)

% 1st hit lick-aligned dffs
[mHitDffFirstLick_early, sHitDffFirstLick_early, ~, hitDffFirstLick_early_ts] = getInterpolatedMeanSemDffFromDffTsCell(rez1.hitDffFirstLick.rs, [-1 1]);
[mHitDffFirstLick_late, sHitDffFirstLick_late, ~, hitDffFirstLick_late_ts] = getInterpolatedMeanSemDffFromDffTsCell(rez2.hitDffFirstLick.rs, [-1 1]);
...
print(fullfile(figSaveDir, strcat(mouseId1, '_', date1, '_', date2, '_dff_lick_Hit_early_late_rs.pdf')), '-dpdf', '-vector', '-bestfit')
...
if isempty(pathToLickWaterRaster)
    ...
    print(fullfile(figSaveDir, strcat(mouseId1, '_', date1, '_', date2, '_dff_cueOn_hit_early_late_RS_lickWaterRaster')), '-dpdf', '-vector', '-bestfit')
end
