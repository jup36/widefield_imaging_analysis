...
%% load and prep data 
SSPath1 = GrabFiles_sort_trials('_regionMask_SS', 0, {fullfile(filePath1, 'Matfiles')});
SSPath2 = GrabFiles_sort_trials('_regionMask_SS', 0, {fullfile(filePath2, 'Matfiles')});

rezSS_early = load(SSPath1{1}, 'rezSS'); 
rezSS_early = rezSS_early.('rezSS'); 

rezSS_late = load(SSPath2{1}, 'rezSS'); 
rezSS_late = rezSS_late.('rezSS'); 

cool = colormap('cool'); % for Go/No-Go

rez1 = load(fullfile(filePath1, 'Matfiles', strcat(header1, '_dff_evtAligned_regionMask.mat')), 'rez');
rez1 = rez1.('rez'); 
[~, stimOn_timepts1] = temporalAlignInterp1(rez1.stimOnDffC.ss(:, 1), rez1.stimOnDffC.ss(:, 2), 0.001);

rez2 = load(fullfile(filePath2, 'Matfiles', strcat(header2, '_dff_evtAligned_regionMask.mat')), 'rez');
rez2 = rez2.('rez'); 
[~, stimOn_timepts2] = temporalAlignInterp1(rez2.stimOnDffC.ss(:, 1), rez2.stimOnDffC.ss(:, 2), 0.001);

rezSS_learn.meanStimOnDffGo = [rezSS_early.meanStimOnDffGng(1, :);   rezSS_late.meanStimOnDffGng(1, :)]; 
rezSS_learn.semStimOnDffGo = [rezSS_early.semStimOnDffGng(1, :);   rezSS_late.semStimOnDffGng(1, :)]; 

%% generate plots
% cue-aligned hit dffs
stimOnFig_learn_SS = plotMeanSem(rezSS_learn.meanStimOnDffGo, rezSS_learn.semStimOnDffGo, stimOn_timepts1, {header1, header2});
...
print(fullfile(figSaveDir, strcat(mouseId1, '_', date1, '_', date2, '_dff_cueOn_hit_early_late_SS')), '-dpdf', '-vector', '-bestfit')
%close(stimOnFig)

% 1st hit lick-aligned dffs
[mHitDffFirstLick_early, sHitDffFirstLick_early, ~, hitDffFirstLick_early_ts] = getInterpolatedMeanSemDffFromDffTsCell(rez1.hitDffFirstLick.ss, [-1 1]);
[mHitDffFirstLick_late, sHitDffFirstLick_late, ~, hitDffFirstLick_late_ts] = getInterpolatedMeanSemDffFromDffTsCell(rez2.hitDffFirstLick.ss, [-1 1]);
...
print(fullfile(figSaveDir, strcat(mouseId1, '_', date1, '_', date2, '_dff_lick_Hit_early_late_ss.pdf')), '-dpdf', '-vector', '-bestfit')
...
if isempty(pathToLickWaterRaster)
    ...
    print(fullfile(figSaveDir, strcat(mouseId1, '_', date1, '_', date2, '_dff_cueOn_hit_early_late_SS_lickWaterRaster')), '-dpdf', '-vector', '-bestfit')
end
