...
for ff = 1:length(filePaths)
    ...
    % cue-aligned go
    mean_stimAlignGoC_RS{ff, 1} = rezRS.meanStimOnDffHitFA(1, :);
    sem_stimAlignGoC_RS{ff, 1}  = rezRS.semStimOnDffHitFA(1, :);  

    % hit-aligned go
    mean_hitFstLickDff_RS{ff, 1} = rezRS.meanHitFstLickDff(1, :); 
    sem_hitFstLickDff_RS{ff, 1} = rezRS.semHitFstLickDff(1, :);
...
mean_stimAlignGoC_RS_norm = cell2mat(cellfun(@(a) a./(max(a)), mean_stimAlignGoC_RS(:, 1), 'UniformOutput', false)); 
% plot
%plotMeanSemColorC(cell2mat(mean_stimAlignGoC_RS(:, 1)), cell2mat(sem_stimAlignGoC_RS), -0.9:0.001:5, learningColorC, id_dateC)
plotMeanSemColorC(mean_stimAlignGoC_RS_norm, cell2mat(sem_stimAlignGoC_RS), -0.9:0.001:5, learningColorC, id_dateC)
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -2:1:5, 'TickDir', 'out', 'YTick', -5:0.2:5); xlim([-1 5]); %ylim([-0.4 1.1])
print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_cueOnAligned_hit_across_session_RS_fullX')), '-dpdf', '-vector', '-bestfit')
xlim([1 4])
print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_cueOnAligned_hit_across_session_RS')), '-dpdf', '-vector', '-bestfit')

%% plot hit-aligned dffs
% min-max normalization
mean_hitFstLickDff_RS_norm = cell2mat(cellfun(@(a) a./(max(a)), mean_hitFstLickDff_RS(:, 1), 'UniformOutput', false)); % plot
% plot
plotMeanSemColorC(mean_hitFstLickDff_RS_norm, cell2mat(sem_hitFstLickDff_RS), -1:0.001:1, learningColorC)
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -1:0.5:1, 'TickDir', 'out', 'YTick', -5:0.2:5); xlim([-1 1]); %ylim([-0.2 1.1])

print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_hitAligned_hit_across_session_RS')), '-dpdf', '-vector', '-bestfit')
