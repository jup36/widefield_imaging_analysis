...
for ff = 1:length(filePaths)
    ...
    filePath_region = GrabFiles_sort_trials('_regionMask_RS', 0, {fullfile(filePath, 'Matfiles')});
    ...
    %load
    load(filePath_region{1}, 'rezRS'); 

    % cue-aligned go nogo
    mean_stimOnGoNogoSvmC_RS{ff, 1} = smooth2a(mean(rezRS.stimOnGoNogoSvm), 0, 4); 
    mean_stimOnGoNogoSvmC_RS{ff, 2} = rezRS.stimOnGoNogoSvmTs; 

    % cue-aligned hit FA
    mean_stimOnHitFaSvmC_RS{ff, 1} = smooth2a(mean(rezRS.stimOnHitFaSvm), 0, 1);
    mean_stimOnHitFaSvmC_RS{ff, 2} = rezRS.stimOnHitFaSvmTs; 

    % cue-aligned hit miss
    mean_stimOnHitMissSvmC_RS{ff, 1} = smooth2a(mean(rezRS.stimOnHitMissSvm), 0, 1);
    mean_stimOnHitMissSvmC_RS{ff, 2} = rezRS.stimOnHitMissSvmTs;   

    % cue-aligned hit cr
    mean_stimOnHitCrSvmC_RS{ff, 1} = smooth2a(mean(rezRS.stimOnHitCrSvm), 0, 1);
    mean_stimOnHitCrSvmC_RS{ff, 2} = rezRS.stimOnHitCrSvmTs; 

    % lick-aligned hit FA
    mean_lickHitFaSvmC_RS{ff, 1} = smooth2a(mean(rezRS.lickHitFaSvm), 0, 1); 
    mean_lickHitFaSvmC_RS{ff, 2} = rezRS.lickHitFaSvmTs; 
...
plotMeanColorC(cell2mat(mean_stimOnGoNogoSvmC_RS(:, 1)), mean_stimOnGoNogoSvmC_RS{1, 2}, learningColorC, id_dateC)
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -2:1:5, 'TickDir', 'out', 'YTick', -5:0.1:5); xlim([-1 5]); %ylim([-0.4 1.1])
print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_cueOnAligned_GoNogo_SVM_RS')), '-dpdf', '-vector', '-bestfit')

%% plot cue-aligned hit fa svm
plotMeanColorC(cell2mat(mean_stimOnHitFaSvmC_RS(:, 1)), mean_stimOnHitFaSvmC_RS{1, 2}, learningColorC, id_dateC)
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -2:1:5, 'TickDir', 'out', 'YTick', -5:0.2:5); xlim([-1 5]); %ylim([-0.4 1.1])
print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_cueOnAligned_HitFa_SVM_RS')), '-dpdf', '-vector', '-bestfit')

%% plot cue-aligned hit miss svm
plotMeanColorC(cell2mat(mean_stimOnHitMissSvmC_RS(:, 1)), mean_stimOnHitMissSvmC_RS{1, 2}, learningColorC, id_dateC)
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -2:1:5, 'TickDir', 'out', 'YTick', -5:0.2:5); xlim([-1 5]); %ylim([-0.4 1.1])
print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_cueOnAligned_HitMiss_SVM_RS')), '-dpdf', '-vector', '-bestfit')

%% plot cue-aligned hit CR svm
plotMeanColorC(cell2mat(mean_stimOnHitCrSvmC_RS(:, 1)), mean_stimOnHitCrSvmC_RS{1, 2}, learningColorC, id_dateC)
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -2:1:5, 'TickDir', 'out', 'YTick', -5:0.2:5); xlim([-1 5]); %ylim([-0.4 1.1])
print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_cueOnAligned_HitCr_SVM_RS')), '-dpdf', '-vector', '-bestfit')

%% plot lick-aligned hit FA svm
plotMeanColorC(cell2mat(mean_lickHitFaSvmC_RS(:, 1)), mean_lickHitFaSvmC_RS{1, 2}, learningColorC, id_dateC)
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -2:1:5, 'TickDir', 'out', 'YTick', -5:0.2:5); xlim([-1 1]); %ylim([-0.4 1.1])
print(fullfile(figSaveDir, strcat(mouseIdC{1}, '_dff_lickAligned_HitFA_SVM_RS')), '-dpdf', '-vector', '-bestfit')
