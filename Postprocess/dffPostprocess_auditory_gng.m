function dffPostprocess_auditory_gng(filePath, varargin)
%This function
%
...
elseif strcmp(p.Results.imageToUse, 'combinedStack') % ome.stacks of same block(sequence) combined
    [file_list_img, ~] = GrabFiles_sort_trials('combinedStack.mat', 1, fileImg(1)); % use GrabFiles_sort_trials to sort both files and folders
    tbytDatDff = struct([]); % This creates a truly empty structure array
    ...
    for ss = 1:length(file_list_img)
        ...
        if isempty(tbytDatDff)
            ...
            % Concatenate the new block of structures to the existing array of structures
            tbytDatDff = [tbytDatDff, tbytDatDffBlock];
        end

        fprintf('Completed combined stack %d/%d\n', ss, length(file_list_img));
    end
    tbytDat = tbytDatDff; 
    clearvars tbytDatDff
