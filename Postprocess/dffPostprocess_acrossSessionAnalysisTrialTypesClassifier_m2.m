...
plotMeanWithoutSem([mean(rezM2_early.lickHitFaSvm); mean(rezM2_late.lickHitFaSvm)], ... 
    rezM2_early.lickHitFaSvmTs, {header1, header2})
title("early vs late m2 hitVsFa SVM (cue onset at time=0)");
set(gca, 'TickDir', 'out')
xlabel('Time (s)'); ylabel('DFF'); set(gca, 'XTick', -1:0.5:1, 'TickDir', 'out', 'YTick', -1:0.1:5); xlim([-1 1]); %ylim([-1 1])
print(fullfile(figSaveDir, strcat(mouseId1, '_', date1, '_', date2, '_dff_lickAlign_hitFA_svm_early_late_m2.pdf')), '-dpdf', '-vector', '-bestfit')
