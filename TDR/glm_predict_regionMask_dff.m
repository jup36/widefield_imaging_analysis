...
fileKeyword = 'red_dff_evtAligned_regionMask'; 
keyword_beh = '_alignedPupilOrofacial.mat';
filePath_dff = cell2mat(GrabFiles_sort_trials(fileKeyword, 0, {filePath_matfiles}));

load(filePath_dff, 'rez')

%%
x = -0.9:0.01:5; % tbytDat_hAligned{2, 1}

% interpolation to get dff*_itp 
dffm1_itp = interp_rez_to_target_time(rez.stimOnDffC.m1, x); 

%%
Ds   = stack_trials_Dffs(dffm1_itp, 'zscore', false);

Ybig = cell2mat(Ds.Yw');         % [(N*nW) x K], raw motif activity
