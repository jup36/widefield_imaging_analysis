function anchor = buildPerMouseLOOAnchors_fromExperts_correctTrials(headerC, glmRezC, trIdC, expertHeaders_perMouse, varargin)
%buildPerMouseLOOAnchors_fromExperts_correctTrials
%   Per-mouse pooled GLM-TDR anchors with LOO/LOSO, built from CORRECT
%   TRIALS ONLY (Hit trials for Go-related groups, CR trials for
%   NoGo-related groups).
%
% WHY THIS VARIANT EXISTS
%   The original buildPerMouseLOOAnchors_fromExperts pools ALL trials of
%   a given stimulus identity (all Go trials, all NoGo trials) when
%   building the SVD-derived anchor axes. Because anchors are only built
%   from "expert" sessions, this is usually a mild issue there (Go~Hit,
%   NoGo~CR at expert performance). But it becomes a real confound as
%   soon as pooled Yhat includes any session where outcome mixture
%   differs from criterion (e.g. borderline-expert sessions with
%   residual FA/miss trials), because Yhat is computed from real
%   recorded-activity-driven design columns, and outcome-related
%   (choice/motor/reward) variance can leak into the same motif
%   dimensions used to define "sensory" axes.
%
%   This function pools ONLY:
%       - Hit trials      -> for Go-related predictor groups
%       - CR trials       -> for NoGo-related predictor groups
%   into the concatenated Yhat used for the anchor-defining SVD, for
%   BOTH axesFull and every axesLOO{m,kk} pooled subset, as well as the
%   single-session reference axes used for alignment.
%
%   Sign-fixing (SignFixTrialPolicy) is left independently configurable,
%   since sign-fixing already only ever operates on expert sessions and
%   the confound there is much smaller; by default it also uses
%   correct-trial-only scoring for consistency.
%
% NEW / CHANGED NAME-VALUE (relative to buildPerMouseLOOAnchors_fromExperts)
%   'TrialPolicy'       : "CorrectOnlyByGroup" (default, and the only
%                         supported value in this variant is enforced by
%                         default; "GoNoGoByGroup" and "all" are also
%                         accepted if you want to A/B test against the
%                         original behavior using this same file).
%   'MinTrialsPerGroup' : 5 (default)
%                         Minimum number of correct trials required for a
%                         given session x group to be INCLUDED in the
%                         pooled SVD. Sessions with fewer than this many
%                         correct trials for that group are SKIPPED for
%                         that group's pool entirely (NOT silently
%                         fallen back to all trials, unlike the
%                         legacy behavior in local_makeMask_/
%                         local_trialMask_fromGroup_ for sign-fixing).
%                         This enforces a "pure correct-trials" pool: a
%                         session either contributes correct trials or it
%                         doesn't contribute at all for that group.
%
% Everything else (OrthMode, SignFix, AlignMode, MultiDimGroups, etc.)
% behaves identically to buildPerMouseLOOAnchors_fromExperts.
%
% LEAN VERSION:
...
p.addParameter('SignFixTrialPolicy',"CorrectOnlyByGroup",@(s)ischar(s)||isstring(s));
...
% NEW: trial pooling policy for the anchor-defining SVD itself
p.addParameter('TrialPolicy', "CorrectOnlyByGroup", @(s)ischar(s)||isstring(s));
p.addParameter('MinTrialsPerGroup', 5, @(x)isnumeric(x)&&isscalar(x)&&x>=0);

% Lean-output bookkeeping (unchanged)
...
opt.SignFixTrialPolicy = string(opt.SignFixTrialPolicy);
...
for m = 1:nM
    ...
    if opt.Verbose && (mod(m, opt.VerboseEvery)==0 || m==1 || m==nM)
        fprintf('[LOOAnchors:correctTrials] mouse %d/%d (%s): found %d/%d expert sessions\n', ...
            m, nM, string(mouseIds{m}), numel(glmList), nK);
    end

    % --- build axesFull (pool all found experts, correct trials only) ---
    if numel(glmList) >= 1
...
if opt.Verbose
    fprintf('[LOOAnchors:correctTrials] done: mice=%d | elapsed=%.1fs\n', nM, toc(t0));
end
end

%% ======================================================================
% Core pooled builder (internal) — CORRECT-TRIALS-ONLY POOLING
% ======================================================================
...
%   - restrict Yhat_g rows to CORRECT trials only for that group
%     (Hit trials for Go-related groups, CR trials for NoGo-related
%     groups), per opt.TrialPolicy, with a MinTrialsPerGroup floor that
%     SKIPS a session's contribution to that group's pool (rather than
%     falling back to all trials) if too few correct trials are present
...
Araw_rows = [];
axisNames = {};
axisMeta  = struct('groupIdx',{},'groupName',{},'pcIdx',{},'sv',{},'expl',{},'nSess',{},'nSessUsed',{});
...
for g = 1:G
    ...
    for s = 1:S
        ...
        Yhat = Xzsub * gr.beta(cols,:); % [M x K], M = N*nW (time-major stacking)

        % ---- CORRECT-TRIALS-ONLY MASKING ----
        assert(isfield(gr,'decBins') && isfield(gr.decBins,'time'), ...
            'glmRez missing decBins.time needed to derive N/nW for trial masking');
        nW = numel(gr.decBins.time);
        M  = size(Yhat,1);

        if rem(M, nW) ~= 0
            warning('buildPooledAxes_concatYhatSVD_:BadStacking', ...
                'M=%d not divisible by nW=%d for session %s (group %s); skipping this session for this group.', ...
                M, nW, headers{s}, char(gName));
            continue;
        end

        N = M / nW;
        [trialMaskN, nCorrect] = local_trialMask_fromGroup_(tr, gName, N, opt.TrialPolicy);

        if nCorrect < opt.MinTrialsPerGroup
            % Pure correct-trials-only pooling: do NOT fall back to all
            % trials. Skip this session's contribution to this group.
            if isfield(opt,'Verbose') && opt.Verbose
                fprintf('    [skip] %s | group=%s | nCorrect=%d < MinTrialsPerGroup=%d -> excluded from pool\n', ...
                    headers{s}, char(gName), nCorrect, opt.MinTrialsPerGroup);
            end
            continue;
        end

        trialMaskM = repmat(trialMaskN(:), nW, 1);  % matches [timeBin1 trials1..N, timeBin2 trials1..N, ...]
        Yhat = Yhat(trialMaskM, :);
        % ---------------------------------------
    ...
    for pc = 1:nKeepEff
        ...
        axisMeta(end).nSessUsed  = nSessUsed;
    end
end
...
if numel(uniqHdr) < numel(hdrFlatS)
    ...
    warning('buildPerMouseLOOAnchors_fromExperts_correctTrials:DuplicateHeaders', ...
...
function [trialMask, nSelected] = local_trialMask_fromGroup_(trId, groupName, N, policy)
% Returns trialMask (logical Nx1) AND nSelected (count of true entries),
% so callers can enforce a minimum-trial-count floor without a second
% pass over the mask.
%
% Supported policy values:
%   "CorrectOnlyByGroup" : Hit trials for Go-related groups,
%                          CR trials for NoGo-related groups.
%                          (Default / intended policy for this variant.)
%   "GoNoGoByGroup"      : Go trials for Go-related groups,
%                          NoGo trials for NoGo-related groups.
%                          (Legacy behavior; kept for A/B comparisons.)
%   "all" / anything else: all trials.
...
if nSelected == 0
    warning('local_trialMask_fromGroup_:EmptyMask', ...
        'Empty trial mask for %s (%s); returning empty mask (caller enforces MinTrialsPerGroup).', ...
        groupName, want);
end
end
