...
Hs = stack_trials_H(tbytDat_hAligned);  % implement as in your decoder
Ybig = cell2mat(Hs.Yw'); 
...
[basis.B_evt, basis.lagsEvt] = make_rcos_basis(8, [-1.0 2.0], step, 'nonlin','linear');
[basis_short.B_evt, basis_short.lagsEvt] = make_rcos_basis(8, [-0.2 0.2], step, 'nonlin','linear');

% optional visualization of basis kernels
figure;
imagesc(basis.lagsEvt, 1:size(basis.B_evt,2), basis.B_evt');  % transpose so lags on y-axis
set(gca, 'YDir', 'normal');    % make lags increase upward
xlabel('Lag (s)');
ylabel('Basis index');
title('Raised Cosine Basis (Event kernels)');

%% 3) Build the design matrix (categorical, task, and behavioral variables)
trI = trialTypeInfoAuditoryGngTbytDat(tbytDat); 
% TT = build_trial_masks(trI); 
% 
% % build a task struct using the categorical labels
% task = struct( ...
%     'trialType', TT.trialType, ...
%     'isGo',      TT.isGo, ...
%     'isNoGo',    TT.isNoGo, ...
%     'isRwd',     TT.isRwd, ...
%     'isPun',     TT.isPun, ...
%     'isLicked',  TT.isLicked);
% [design.tt, design.trialTypeNames, design.trialTypeinfo] = trialtype_to_design(task.trialType, 'ref', 'Hit'); 
% N = #trials, nW = #time bins
%X_TT_sparse = sparse(repmat(design.tt, nW, 1));      % (N*nW) × P
...
[toneOn_Go_rc, toneOn_Go_rc_names] = convolve_events_with_basis(toneOn_Go_tbyb, basis, 'toneOnGo'); 
...
[toneOff_Go_rc, toneOff_Go_rc_names] = convolve_events_with_basis(toneOff_Go_tbyb, basis, 'toneOffGo'); 
...
[toneOn_NoGo_rc, toneOn_NoGo_rc_names] = convolve_events_with_basis(toneOn_NoGo_tbyb, basis, 'toneOnNoGo'); 
...
[toneOff_NoGo_rc, toneOff_NoGo_rc_names] = convolve_events_with_basis(toneOff_NoGo_tbyb, basis, 'toneOffNoGo'); 
...
[lick_rc, lick_rc_names] = convolve_events_with_basis(combinedLicksVid_tbyb, basis_short, 'lick'); 

% outcome
water_tbyb = events_to_design(tbytDat, Hs.winBounds, 'water', 'absTime', true); % trial x nW
[water_rc, water_rc_names] = convolve_events_with_basis(water_tbyb, basis, 'water'); 

airpuff_tbyb = events_to_design(tbytDat, Hs.winBounds, 'airpuff', 'absTime', true); % trial x nW
[airpuff_rc, airpuff_rc_names] = convolve_events_with_basis(airpuff_tbyb, basis, 'airpuff'); 
