function glmRez = tryLoadGlmRez(filePathBase, mId, header, glmFileKeyword)
% Locate and load one session's glmRez (beta, X_names), mirroring the
% same mId -> header -> task -> Matfiles discovery chain used elsewhere in
% this pipeline. Returns [] (not an error) if anything in the chain is
% missing -- caller logs the reason.
glmRez = [];
