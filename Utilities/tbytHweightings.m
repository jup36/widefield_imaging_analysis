function tbytHweightings(hCell, tbytDat, timeWin, timeStep)
% timeWin = [-0.9 5]; 
% timeStep = 0.01; 

nMotifs = size(hCell{1,1}, 1); 
...
hIntAlignedC = cell(1, numel(tbytDat)); 

for t = 1:numel(tbytDat)
    hIntAlignedC{t, 1} = NaN(nMotifs, length(tint));
    blockId = blockIds(t);
    ...
    % Preallocate
    hmatTrInt = zeros(size(hmatTr, 1), numel(tintI));
