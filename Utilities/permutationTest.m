...
addParamValue(p, 'parametric',1, @isnumeric);
...
if p.Results.parametric
    allobservations = [sample1, sample2];
    observeddifference = nanmean(sample1) - nanmean(sample2);
    effectsize = observeddifference / nanmean([nanstd(sample1), nanstd(sample2)]);
...
else %if not parametric, use a permutation test on their medians
    allobservations = [sample1, sample2];
    observeddifference = nanmedian(sample1) - nanmedian(sample2);
    effectsize = NaN;
    ...
    for n = 1:permutations
        ...
        % saving differences between the two samples
        randomdifferences(n) = nanmedian(randomSample1) - nanmedian(randomSample2);
    end
    if showprogress, delete(w); end
