function Scotty_RefitCuratedStaticBasisMotifs_Swarm_JP(filePathImg, fileKeyword, basis_dir, parameter_class, save_dir)
%Camden MacDowell
...
file_processed_folder = find_keyword_containing_folder(compatiblepath([gp.local_bucket gp.processing_intermediates]), fileheader, 'recursive', false); 
...
for cur_chunk = 1:size(temp.data_test,3)
    script_name = WriteBashScriptWinScotty(sprintf('refitchunk%d',cur_chunk),'RefitCuratedStaticBasisMotifs_JP',{ConvertWinToBucketPath(file_processed),...
