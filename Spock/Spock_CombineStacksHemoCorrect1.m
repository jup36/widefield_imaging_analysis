function Spock_CombineStacksHemoCorrect1(folder_path,save_fn,parameter_class)
% Note that this function 'Spock_CombineStacksHemoCorrect1' uses
% 'HemodynamicCorrectionFF1', which uses std filter to interpolate the pixels with 'saturated' 
% artifacts within the hemodynamic correction frames. The function
% 'inpaint_nans' conducts the interpolation with the 'spring metaphor'
% method (modified by Junchol Park 5/7/25). 
% 'HemodynamicCorrectionFF1'
...
% parameter_class = 'general_params_example';  
    ...
    try
        ...
        if numel(unique(opts.wavelength_pattern))>1 %if multiple wavelengths used
           [dff, dff_b, ~] = HemodynamicCorrectionFF1(stack, opts); 
